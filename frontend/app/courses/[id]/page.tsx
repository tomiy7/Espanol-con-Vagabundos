...
//         `${process.env.NEXT_PUBLIC_DIRECTUS_URL}/items/lessons?filter[course_id][_eq]=${courseId}&sort=order`,
//         { cache: "no-store" }
//     );
//     const json = await res.json();
//     return json.data;
...
async function getLessons(courseId: string): Promise<Lesson[]> {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_DIRECTUS_URL}/items/lessons?filter[course_id][_eq]=${courseId}&filter[visible][_eq]=true&sort=order`,
        { cache: "no-store" }
    );
    const json = await res.json();
    return json.data;
